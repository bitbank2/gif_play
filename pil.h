...
/* MPEG data structure */
typedef struct _mpegdata
{
// variables needed to walk through the mpeg stream
   int iWidth;
   int iHeight;
   ...
   unsigned char *pFileBuf; // 32k for working with file data
   unsigned char *pVideo; // compressed video buffer
   unsigned char *pAudio; // compressed audio buffer
   int iFRefFrame; // frame number of forward reference frame
   signed short *pBRef[3]; // backward reference frame
   signed short *pFRef[3]; // forward reference frame
   unsigned short *pACTables;
   ...
   int iVBVBufferSize;
   int iCurrentFrame;
   PILBOOL bPacketized; // indicates if the data stream is in packets or is raw video (single stream)
   unsigned char ucIntraQuant[64];
   unsigned char ucNonIntraQuant[64];
   signed char cMVPredX[128]; // current and previous motion vector predictors
   signed char cMVPredY[128]; // needed for h.263
   unsigned char ucPelAspect, cQuantizerScale;
   unsigned short usYUVRGB[65536]; // lookup table for colorspace conversion
} MPEGDATA;

// Page metadata (TIFF & JPEG for now)
// The data needs to have only relative pointers so
// that it can be copied to a new buffer without corrupting it
typedef struct _pilmeta
{
int iObjectCount;
int iTag[MAX_META_COUNT]; // EXIF/TIFF tag number
unsigned char ucTagType[MAX_META_COUNT]; // data type of each tag
int iTagOffset[MAX_META_COUNT]; // offset in the blob area to the tag's data
int iTagDataSize[MAX_META_COUNT]; // size of the data
int iBlobSize; // size of all of the data in the blob
int iAllocatedSize; // current size of the metadata - reduces re-allocation
unsigned char ucBlob[4]; // starting point of the blob data
} PIL_META;
...
unsigned short sQuantTable[8*DCTSIZE2];
...
unsigned char ucHuffACDCBuf[4*0x100 + 4*0x1000]; // pre-allocate the maximum space for the Huffman decode tables
...
int iLinesDecoded; // number of scanlines successfully decoded when there is an error
unsigned char *pData;		// pointer to image data
int iPitch;       // The stride of a row
int iOffset;		// offset to start of image data
int iXres, iYres;	// Resolution in dots per inch
int iDataSize;		// Size of the data
int iX, iY;       // offsets to handle GIF properly
int iCX, iCY;     // used for GIF animation
int iFrameDelay;  // display delay in milliseconds and EXIF subIFD offset
int iRepeatCount; // GIF animation repeat count (NETSCAPE app extension value)
...
int iTransparent; // transparent color (single alpha)
int iBackground;  // background color index or RGB888 value (for APNG)
int iPageWidth, iPageHeight; // GIF page size & JPEG EXIF true size
unsigned char *pPalette;	// pointer to color palette for 1, 4 & 8 bpp images
unsigned char *pLocalPalette; // GIF images can have both a global and local color table for a single frame
// Strip info
int  iStripCount;          // Number of strips in the page
uint32_t *plStrips;            // Pointer to strip offsets
uint32_t *plStripSize;         // Pointer to strip size info
JPEGDATA *pJPEG;           // used for motion-jpeg - need to keep this structure around
void *pAnnotations;        // Annotation data overlay
int iAnnotationSize;
int  iRowCount;            // Rows per strip
uint32_t ulIOThreadID;  // for multithreaded operations
int iDataAvailable;        // pre-loaded data for multithreaded operations
int iFileSize;             // needed to know when to stop reading
int iHighWater;            // current high water mark for the compressed data
int hEvent;               // mutex event to throttle file reading
...
MPEGDATA *pMPEG;           // pointer to MPEG stream info
PIL_META *pMeta;           // Page metadata (TIFF & JPEG for now)
...
char szMake[128];           // camera make
char szModel[128];          // camera model
pilcomps cCompression;		// Compression type
char cPhotometric;			// photometric interpretation (or brightness for JPEG)
char cBitsperpixel;			// Supported values = {1,4,8,16,24,32}
char cBppStored;           // actual used bits
...
// Variables for managing a dynamically growing buffer (e.g. for encoding)
unsigned char *pHighWater; // current highwater mark
int iCurrentBufferSize; // size of the buffer in bytes
int iBufferIncrement; // how much to grow the buffer the next time
} PIL_PAGE;
...
PILRGBQUAD rgbColor2;                             // The secondary color; for example, the color of the text of
                                                                // an Attach-a-Note.
PILBOOL bHighlighting;                     // TRUE The mark is drawn highlighted. Highlighting
                                                                // performs the same function as a highlighting marker on a
                                                                // piece of paper. Valid only for lines, rectangles, and
                                                                // freehand.
PILBOOL bTransparent;                      // TRUE The mark is drawn transparent. A transparent
...
PILBOOL bVisible;                                // TRUE The mark is currently set to be visible.
...
typedef struct _pilfile {
   ...
   JPEGDATA *pJPEG;           // Precalc'd tables for JPEG + video files
	...
	pilcomps cCompression;     // for video
...
int PILDraw(PIL_PAGE *pPage, PIL_VIEW *pView, PILBOOL bTopDown, void *pGammaBrightness);
...
void PILFreeMPEGStruct(MPEGDATA *pMPEG, PILBOOL bMPEG);
int PILReadMeta(PIL_PAGE *pPage, PILBOOL bByIndex, int iTagIndex, unsigned char *pucType, unsigned char *pOut, int *pLength);
...
PILBOOL PILIsGray(PIL_PAGE *inpage);
...
PILBOOL PILTest(TCHAR *szFileName);
int PILAutoCrop(PIL_PAGE *pPage, PILBOOL bBlackBorder);
